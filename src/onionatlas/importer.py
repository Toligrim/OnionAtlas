...
def _ensure_page(
    ...
    *,
    ...
    row = connection.execute(
        "SELECT id, current_text_hash FROM pages WHERE canonical_url = ?", (canonical.url,)
    ).fetchone()
    if row is not None:
    ...
    cursor = connection.execute(
        """
        INSERT INTO pages(
            service_id, canonical_url, path, query, first_seen_at,
...
def _sync_fts(
    ...
    *,
    ...
    connection.execute(
        "INSERT INTO pages_fts(page_id, title, description, h1, body) VALUES (?, ?, ?, ?, ?)",
        (page_id, title or "", description or "", h1 or "", body),
    )


def _insert_revision_if_changed(
    connection: sqlite3.Connection,
    *,
    page_id: int,
    ...
    previous = connection.execute(
        "SELECT id FROM page_revisions WHERE page_id = ? ORDER BY id DESC LIMIT 1",
        (page_id,),
    ).fetchone()
    connection.execute(
        """
        INSERT INTO page_revisions(
            page_id, observed_at, text_hash, title, description, h1,
            normalized_text, supersedes_revision_id
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (
            page_id,
            observed_at,
            result.document.text_hash,
...
def import_result(
    ...
    *,
    ...
    existing = connection.execute(
        "SELECT id, page_id FROM fetches WHERE attempt_id = ?", (result.attempt_id,)
    ).fetchone()
    if existing is not None:
        return ImportOutcome(
            ...
            True,
            int(existing["id"]),
            int(existing["page_id"]) if existing["page_id"] is not None else None,
        )
    ...
    try:
        ...
        if row["state"] != "leased":
            raise ImportRejected("task is not currently leased")
        if row["lease_owner"] != result.worker_id:
            raise ImportRejected("worker does not own lease")
        ...
        if not result.success:
            ...
            fetch_id = _insert_fetch_failure(
                connection,
                result=result,
                service_id=task_service_id,
            )
            service_status = _error_status(result.error.error_class)
            next_attempt = to_iso(now_dt + _retry_delay(int(row["attempt_count"])))
            connection.execute(
                """
                UPDATE services
                SET last_seen_at = ?, last_offline_at = ?, current_status = ?,
                    crawl_count = crawl_count + 1,
                    failure_count = failure_count + 1,
                    consecutive_failures = consecutive_failures + 1,
                    next_recrawl_at = ?, updated_at = ?
                WHERE id = ?
                """,
                (
                    now,
            ...
            connection.execute(
                """
                UPDATE frontier
                SET state = 'retry', next_attempt_at = ?, last_error_class = ?,
                    lease_owner = NULL, lease_id = NULL, attempt_id = NULL,
        ...
        page_id, _page_created, previous_hash = _ensure_page(
            connection,
            service_id=final_service_id,
            final_url=final.url,
            now=now,
        )
        _insert_revision_if_changed(
            connection,
            page_id=page_id,
        ...
        connection.execute(
            """
            UPDATE pages
            SET last_seen_at = ?, last_fetch_at = ?, current_status_code = ?,
                current_content_type = ?, current_title = ?, current_description = ?,
                current_h1 = ?, current_text = ?, current_text_hash = ?,
                current_body_bytes = ?, current_requires_javascript = ?, updated_at = ?
            WHERE id = ?
            """,
            (
                now,
                ...
                result.document.normalized_text,
                result.document.text_hash,
                result.response.body_bytes,
                int(result.document.requires_javascript),
                now,
                page_id,
            ),
        )
        fetch_cursor = connection.execute(
            """
            INSERT INTO fetches(
                page_id, service_id, task_id, attempt_id, lease_id, worker_id,
                requested_url, final_url, started_at, finished_at, status_code,
                content_type, charset, body_bytes, normalized_text_bytes, elapsed_ms,
                redirect_count, content_hash, text_hash, success, created_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?)
            """,
            (
                page_id,
                ...
                result.response.body_bytes,
                result.response.normalized_text_bytes,
                result.response.elapsed_ms,
                len(result.response.redirect_chain),
        ...
        _sync_fts(
            ...
            body=result.document.normalized_text,
        )
        ...
        for link in result.links:
            try:
                ...
                continue
            target_service_id, target_created = ensure_service(
                connection, canonical_link.onion_host
            )
            ...
            if int(row["depth"]) < max_depth:
                ...
                enqueue_url(
                    ...
                    priority=max(-100.0, float(row["priority"]) - 1.0),
                    source_evidence_id=int(evidence_cursor.lastrowid),
                )

            existing_link = connection.execute(
                "SELECT id FROM links WHERE source_page_id = ? AND target_url = ?",
                (page_id, canonical_link.url),
            ).fetchone()
            if existing_link is None:
                connection.execute(
                    """
                    INSERT INTO links(
                        source_page_id, target_url, target_service_id, anchor_text,
                        first_seen_at, last_seen_at, times_seen, last_fetch_id
                    ) VALUES (?, ?, ?, ?, ?, ?, 1, ?)
                    """,
                    (
                        page_id,
                ...
                connection.execute(
                    """
                    UPDATE links
                    SET target_service_id = ?, anchor_text = COALESCE(?, anchor_text),
        ...
        if final_service_id == task_service_id:
            connection.execute(
                """
                UPDATE services
                SET last_seen_at = ?, last_online_at = ?, current_status = 'online',
                    current_title = ?, current_description = ?, current_content_hash = ?,
                    current_homepage_page_id = CASE
                        WHEN ? THEN ? ELSE current_homepage_page_id END,
                    crawl_count = crawl_count + 1,
            ...
            # The requested service was reachable and redirected to another onion service.
            # Keep the source service operational state, but never attach the target
            # service's page as the source service's homepage/current document.
            connection.execute(
                """
                UPDATE services
                SET last_seen_at = ?, last_online_at = ?, current_status = 'online',
                ...
                WHERE id = ?
                """,
                (now, now, next_recrawl, now, task_service_id),
            )
            connection.execute(
                """
                UPDATE services
        ...
        connection.execute(
            """
            UPDATE frontier
            SET state = 'done', next_attempt_at = NULL, last_error_class = NULL,
                lease_owner = NULL, lease_id = NULL, attempt_id = NULL,
                leased_at = NULL, lease_expires_at = NULL, updated_at = ?
            WHERE id = ?
            """,
